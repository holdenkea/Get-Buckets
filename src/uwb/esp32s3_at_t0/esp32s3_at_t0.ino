...
void loop()
{
    ...
    while (SERIAL_AT.available() > 0)
    {
        ...
        else if (c == '\n' || c == '\r')
        {
            ...
            if (response.startsWith("AT+RANGE=tid")) {
                int rangeStart = response.indexOf("range:(");
                if (rangeStart != -1) {
                    rangeStart += 7; // Move past 'range:('
                    int rangeEnd = response.indexOf(")", rangeStart);
                    String rangeValues = response.substring(rangeStart, rangeEnd);
                    // Split and pick the first 3
                    int firstComma = rangeValues.indexOf(',');
                    int secondComma = rangeValues.indexOf(',', firstComma + 1);
                    int thirdComma = rangeValues.indexOf(',', secondComma + 1);
            
                    String r0 = rangeValues.substring(0, firstComma);
